...
import { IReleaseLetter } from "../release-letter/release-letter.interface";

export const releaseLetterController = {
  getOfferLetterAll: catchAsync(async (req, res) => {
    const result = await experienceLetterService.getExperienceLetterAll(
      req.query
    );
    sendResponse(res, {
  ...
  async createBulkOfferLetter(req: Request, res: Response) {
    ...
    const results = await experienceLetterService.createBulkExperienceLetters(
